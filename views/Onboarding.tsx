...
import { REQUIRED_DOCS_MAP, INDUSTRY_DOCS, COUNTRIES, NACE_CODES, FINANCIAL_PRODUCTS, DEMO_SCENARIOS } from '../constants';
import { performRiskAnalysis } from '../services/geminiService';
import RiskBadge from '../components/RiskBadge';
import { ArrowRight, Upload, AlertCircle, Loader2, FileCheck, Search, ChevronRight, Check, Briefcase, MapPin, Mail, Globe, Zap, Bot, Sparkles } from 'lucide-react';
...
const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
  ...
  const handleInputChange = (field: string, value: string) => {
    ...
    // Reset docs if industry changes (simulating dynamic rule trigger)
    if (field === 'industry') {
      updateDocRequirements(entityType as EntityType, value);
    }
  };

  const updateDocRequirements = (type: EntityType, industry?: string) => {
    let docs = REQUIRED_DOCS_MAP[type] || [];
    
    // Add industry specific docs based on partial string match with NACE codes
    if (industry && INDUSTRY_DOCS[industry]) {
      docs = [...docs, ...INDUSTRY_DOCS[industry]];
    }

    setRequiredDocs(docs.map((d, i) => ({
      id: `doc-${i}`,
      name: d.name,
      description: d.desc,
      uploaded: false
  ...
  const handleScenarioClick = (scenario: any) => {
    ...
    updateDocRequirements(scenario.type, scenario.data.industry);
    setStep(2);
  };
      ...
      <div className="max-w-4xl mx-auto bg-white p-8 rounded-xl shadow-sm border border-slate-200">
        <div className="flex items-center space-x-3 mb-8 pb-4 border-b border-slate-100">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Complete all fields to facilitate risk assessment and due diligence.</p>
          </div>
        </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {isIndividual ? (
                  <>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-slate-700 mb-1">Tax ID / SSN (Optional)</label>
                      <input
                        type="text"
  ...
  const renderStep3 = () => {
    ...
    return (
      <div className="max-w-4xl mx-auto bg-white p-8 rounded-xl shadow-sm border border-slate-200">
        ...
        <p className="text-slate-500 mb-6">Based on the entity type, industry, and selected product, the following documents are mandatory.</p>

        <div className="space-y-4 mb-8">
          {requiredDocs.map((doc) => (
            <div key={doc.id} className={`p-4 border rounded-lg flex items-center justify-between ${doc.uploaded ? 'bg-emerald-50 border-emerald-200' : 'bg-white border-slate-200'}`}>
  ...
  const renderStep4 = () => {
    ...
    const handleFinish = () => {
      const newEntity: EntityProfile = {
        ...
        approvedBy: isClean ? 'AI' : undefined
      };
